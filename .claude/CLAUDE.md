...
- **티어 분기(Phase 3~4)**: `_classify_tier(request, estimated_tokens)`가 long/complex/simple 판단 → `AUTO_CANDIDATES_BY_TIER[tier]` 선택. 토큰 추정은 `_auto_route`에서 1회만 수행해 전달.
  ...
  - **agentic(Phase 6)**: 도구 '정의'만으로 `_AGENTIC_TOOL_TOKENS(1500)` 이상 → 코딩 에이전트 하네스(opencode·Claude Code류)로 본다. 후보 [ollama/qwen3:14b, gemini-2.5-flash] — 다른 티어와 같은 **로컬 우선**(auto의 기본은 비용 0인 로컬). 대화가 커져 `추정 입력 + max_tokens`가 로컬 usable 창을 넘으면 컨텍스트 필터가 로컬을 떨어뜨려 1M Gemini로 승격된다. 티어를 따로 두는 이유는 후보 순서가 아니라 **관측**이다(`reason=tier=agentic`으로 '계속 커질 세션'을 로그에서 구분). 도구 1~2개짜리 평범한 function calling(수백 토큰)은 complex.
