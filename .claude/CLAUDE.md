...
여러 LLM provider(Ollama, Anthropic)를 단일 OpenAI 호환 API로 묶는 게이트웨이 서버.

## 목적

- 에이전트/클라이언트가 provider를 몰라도 되도록 추상화
- 모델 이름만 바꿔서 Ollama ↔ Anthropic 전환
- Oracle 온프레미스 서버에서 실행하는 것을 전제로 설계

## 기술 스택

- **Python** + **FastAPI** (ASGI 서버: uvicorn)
- **httpx** — Ollama 비동기 HTTP 프록시
...
├── config.py      — Settings (ANTHROPIC_API_KEY, OLLAMA_BASE_URL)
├── models.py      — ChatCompletionRequest 등 OpenAI 호환 Pydantic 모델
├── registry.py    — 라우팅 결정: ModelSpec / RouteDecision / MODELS / ALIASES / DEFAULT_MODEL / resolve()
├── service.py     — ProviderPool(인스턴스 재사용) + fallback 실행 + 에러 분류
└── providers/
...
- **silent fallback 주의**: 등록된 claude 모델이 404/연결오류거나 키 미설정이면 fallback 체인의 로컬 Ollama로 조용히 떨어질 수 있다. 실제 사용된 모델은 응답 `model` 필드로 확인.
